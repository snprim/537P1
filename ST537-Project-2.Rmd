...
* Use the caret package to do training and predition?
* Please do condense any tables or figures if you see a way to do so.
* Longitudinal analysis: consider how to show whether time point 2 is the peak and if the difference is significant  
...
[need to list $\mathbf{D}$ and $\Sigma$]
...
lda <- lme(fixed = binding ~ Time_Point + D1 + D1:Time_Point + D2 + D2:Time_Point,
           random = ~ Time_Point | ID, data = dataLDA, method = "REML")
summary(lda)
```
