...
From Drug (treatment) vs. Binding plot, we can observe for each type of treatments, there are simliar spread of antibody binding. We don't see any treatment leads to especially higher or lower antibody binding affinity in general. After split the doses among each treatments, we see group 3 in Drug 1 (treatment 1) and group 6 in Drug 2 (treatment 2) have a few antibody observations and most of them a low binding affinity. There are smaller number of antibody isolated from group 3 and 6 is due to limit of funding and labor resources for this study. Samples performed in these two groups are time point 1, which is not suprised that mostly low binding affinity.   


## Outlier detection

Before we go into analyses, notice may have outlier in LCDR3 variable. The median and mean LCDR3 lenght are around 9 amino acids, but the maxium LCDR3 is 47 amino acides. From the following plots analysis, we can clearly observe the existing of outlier. 
...
```
Row 972 from `Data2` is in fact an outlier, as shown in the summary and plots above. The value for `L_CDR3` is quite unlikely. According to the experience, this is caused by the error during sequencing data analysis, Since we don't have the raw sequencing data and related software resources to re-analysis this particular data point, we decide to remove it and will use the new dataset `Data3`. It will not affect the conclusion of statistics analysis or inferences.   
