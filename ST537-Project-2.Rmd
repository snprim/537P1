...
`HMuFreq` and `LMuFreq` are calculated by dividing `H_Substitution` by `H_VBase` for heavy chain and similarly for light chain. These two variables show how much the antibodies mutate. A higher mutation rate is usually indicative of better virus neutralization. Below we see some comparison of mutation rate between heavy chain and light chain. (Kappa and Lambda are two kinds of light chain.) [Kan, is there a reason to split up light chain into Kappa and Lambda? Could we simply plot heavy chain vs. light chain?]  
...
# [I don't know what Kappa and Lambda Chains are, are we evaluating them for our research questions?  We should describe how they fit into the research questions if they are, or otherwise omit them for being extra detail.]   
```{r H/K/L Mutation Rate}
...
g7 <- ggplot(MutSubData, aes(x=Time_Point, y=HMuFreq)) 
  g7 + geom_point(outlier.shape = NA) +
    ggtitle("H_Mutation") +
...
First, we use manova to test effects. Since we have a large sample size (n = 2464) [I don't think this is an accurate sample size.  Some of the isotypes only have 1-10 samples in some of the time points.  I think we should trim to IgG and IgM, can possibly include IgD], we can assume normality. In the output, we can see that the main effects of `Drug` and `Isotype` and the interaction effects are all significant.  
