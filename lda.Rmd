...
```{r, AIC-tab}
kable(data.frame(AIC(fit.gls, fit.a1, fit.a2), BIC(fit.gls, fit.a1, fit.a2)), caption = "AIC and BIC for three models")
```

As shown in Table \@ref(tab:AIC-tab), the model `fit.a2` (random intercept and slope, AR1 correlation, unequal variances) has the lowest AIC And BIC, so it seems the bets model. We now check residuals for both models.  
...
Now we would like to know if the slopes between Time_Point 1 and 2 and between Time_Point 2 and 3 equal zero. $H_0: \text{ slope of } S2=0 \text{ and slope of } S2=0$, which means $H_0: -\beta_0 -\beta_1 + \beta_4 + 2\beta_5 = 0 and \beta_5=0$  

Thus, we can check for two tests: $$\mathbf{L_1 \beta}=0$$ where $\mathbf{L_1}=(-1, -1, 1, 2)$ and $\mathbf{\beta}=(\beta_0, \beta_1, \beta_4, \beta_5)^T$ and $$\mathbf{L_1 \beta} = 0$$ where $\mathbf{L_2}=(0, 0, 0, 1)$ and $\mathbf{\beta}=(\beta_0, \beta_1, \beta_4, \beta_5)^T$  

```{r, S2S3-slope}
L1 <- matrix(c(-1, -1, 1, 2), byrow = TRUE, nrow = 1)
L2 <- matrix(c(0, 0, 0, 1), byrow = TRUE, nrow = 1)
test1 <- anova.lme(fit.a2, L = L1, adjustSigma = TRUE)
test2 <- anova.lme(fit.a2, L = L2, adjustSigma = TRUE)
kable(data.frame(rbind(test1, test2)), caption = "Inference of S2 ad S3 slopes")
```

As shown in Table \@ref(tab:S2S3-slope), both of the slopes in S2 and S3 have very small p-values, indicating that Time_Point 2 has significantly higher Binding rate than Time_Point 1 (since the slope is positive) and that Time_Point 3 has significantly lower Binding rate than Time_Point 2 (since the slope is negative). In other words, Time_Point 2, when the monkeys had received two vaccines, had the highest Binding rate.  
