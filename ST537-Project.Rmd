...
```{r}
subset1 <- Data2 %>% filter(Time_Point == 1) %>% filter(MonkeyID == 6132) %>% select(HMuFreq, LMuFreq, H_CDR3, L_CDR3) %>% pairs()
subset1 <- Data2 %>% filter(Time_Point == 1) %>% select(HMuFreq, LMuFreq, H_CDR3, L_CDR3) %>% pairs()
```
...
fit.manova <- manova(cbind(Data2$L_CDR3, Data2$LMuFreq, Data2$H_CDR3, Data2$HMuFreq) ~ ID + trt+ tp + it)
summary(fit.manova)
```
